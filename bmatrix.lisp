...
(symbol-macrolet
  ...
  (defstruct (basis-matrix
    ...
    (u-cols-in-rows   err :type (simple-array (simple-array fixnum 1) 1))
    (u-cis-in-rows    err :type (simple-array (simple-array fixnum 1) 1))
    (u-row-lengths    err :type (simple-array fixnum 1))
    (update-row-is    err :type (simple-array fixnum 1))
    (update-row-vals  err :type (simple-array rational 1))
    (update-row-length  0 :type fixnum)
...
(defun make-basis-matrix (&key (m -1) (lp nil) (max-l-file-size 2000) (row-col-max 4))
  (when lp
  ...
  (let* ((hsv0 (make-hsv))
	 ...
	 (bm (%make-basis-matrix
	    ...
	    :u-cols-in-rows   (make-array m :initial-element a0 :element-type '(simple-array fixnum 1))
	    :u-cis-in-rows    (make-array m :initial-element a0 :element-type '(simple-array fixnum 1))
	    :u-row-lengths    (make-array m :initial-element 0 :element-type 'fixnum)
	    :update-row-is    (make-array m :initial-element -1 :element-type 'fixnum)
    ...
    (dotimes (k m bm)
      (setf (aref (basis-matrix-u-columns bm) k)   (make-hsv)
	    ...
	    (aref (basis-matrix-u-cols-in-rows bm) k)  (make-array m :initial-element -1 :element-type 'fixnum)
	    (aref (basis-matrix-u-cis-in-rows bm) k)  (make-array m :initial-element -1 :element-type 'fixnum)
  ...
  (let ((m (basis-matrix-size bm)))
    (dotimes (k m)
      (setf (aref (basis-matrix-fill-ins bm) k) 0
	    ...
	    (aref (basis-matrix-u-row-lengths bm) k) 0
