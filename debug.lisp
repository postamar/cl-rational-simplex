...
(defun check-lu (lp bm bh)
  (when *checks*
	...
	(dotimes (i m)
	  (dotimes (j m)
	    (let ((ip (aref (basis-matrix-pi->i bm) i))
		  (jp (aref (basis-matrix-pj->j bm) j)))
...
(defun simplex-profile ()
  ...
  (profile simplex-compute-flip-ftran-rhs)
