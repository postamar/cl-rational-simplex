...
(defun fill-basis-matrix (bm lp basis-header)
  ...
  (dotimes (j (basis-matrix-size bm))
	...
	(setf (lu-eta-matrix-coef u_j)    (column-coef col)
	      (lu-eta-matrix-col-ref u_j) col-ref)
	(dotimes (k n-nz)
	  (let ((i (aref (lp-active-row-inds lp) (aref col-row-refs k)))
		(ci (length (lu-eta-matrix-is u_j)))
		(val     (aref col-vals k)))
	    (unless (= -1 i)
	      (vector-push-extend i   (lu-eta-matrix-is  u_j))
	      (vector-push-extend val (lu-eta-matrix-vis u_j))
	      (vector-push-extend 0.0 (lu-eta-matrix-vfs u_j))
	      (incf (aref (basis-matrix-col-nnz bm) j))
	      (incf (aref (basis-matrix-row-nnz bm) i))
	      (vector-push-extend i   (aref (basis-matrix-col-is bm) j))
	      (vector-push-extend j   (aref (basis-matrix-row-js bm) i))
	 ...
	 (map-pivot-bucket
	  #'(lambda (pivot-key ci)
	      (let* ((i (floor pivot-key m))
		     ...
		     (vi (aref (lu-eta-matrix-vis u) ci)))
		(if (zerop vi)
		    (setf is-zero t
			  zero-i i
			  zero-j j
